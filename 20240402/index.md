...
<br>
## 8.
英特尔的硬件反馈接口（HFI）驱动程序将在今年夏天的Linux 6.10内核中更有效地运行。
<br>
您可能还记得二月份讨论过的Linux内核补丁，只有在实际使用用户空间软件组件（如Intel Speed Select或Intel Low Power Daemon实际想要使用HFI事件时）才启用HFI驱动程序。根据英特尔的Linux工程师们的说法，当没有用户空间组件在积极利用它时，跳过英特尔HFI驱动程序活动可以节省“大量的CPU周期”。
<br>
英特尔的硬件反馈接口用于向操作系统传达每个CPU核心的性能/能效能力信息。Linux可以利用英特尔HFI进行更好的任务分配，这对于采用混合设计的最新英特尔Core处理器尤为重要。
<br>
![图片暂时迷路了！！:(](img/8.png)
<br>
英特尔工程师没有量化CPU周期节省的影响，但现在的消息是，这些补丁预计将在Linux 6.10中首次亮相。上周，只在需要时启用英特尔HFI的补丁已经排队进入电源管理子系统的“linux-next”分支。随着代码达到这个里程碑，它将进入下一个合并窗口（v6.10），除非在最后一刻出现问题。
<br>
*“当用户空间处理程序存在时，启用和禁用硬件反馈接口（HFI）。例如，当intel-speed-select或Intel Low Power守护进程正在运行并订阅热网链接事件时，启用HFI。当用户空间处理程序退出或取消订阅热网链接事件时，禁用HFI。”*
<br>
因此，现在它位于Linux-pm.git的linux-next分支中，等待5月份6.10合并窗口的开启。
<br>
